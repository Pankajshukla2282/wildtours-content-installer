...
| `12-SEED-DATA.json` | Machine-readable content import matching plugin CPT/SCF fields — plus the `navigation` tree (header menu from `01-URL-MATRIX-NAVIGATION.md` section C), written to the "PWT Navigation" options page (`options_nav_items_*`, `options_header_cta_*`, `options_topbar_text`) |
...
- **Media.** 125 attachments referenced; 20+ are 2026-08 stock images (`Panna-Tiger-Reserve-04*.jpg`, `Jeep-Safari*.jpg`, `Panna-national-park*.jpg`, `Khajuraho-temples.jpg`, `Bengal-tiger-…-09*.jpg`, `Pandava-Caves-1-of-1*.jpg`, etc.) already live and used as featured/hero images. Legacy 2020 photos (`waterfall.jpg`, `baldau_mandir.jpg`, `water.jpg`, `jugalkishor_mandir.jpg`, `resized-500xsquare-1.png` logo) are reused for zones/attraction pages. The legacy hero `.mp4` (`VID-20230708-WA0041.mp4`) is **not** present on the live server, so the homepage hero now uses the live tiger photo as a cover image; four brand illustrations (`ken-river-boat-safari.svg`, `panna-kingfisher.svg`, `panna-jungle-lodge.svg`, `panna-resort-room.svg`) were generated in `import-media-svg/` for the spots with no real photo — upload them to `/wp-content/uploads/2026/08/` (see `10-MEDIA-SPEC-SHEET.md`).
...
4. **Import PWT content** — run `12-SEED-DATA.json` through the plugin's seed importer (`Admin → Content Forms → Seed Starter Content`) or the WP importer; alternatively paste each `pwt_*` record via `Admin → PWT Quick Content Forms`. Verify taxonomies attach correctly (`pwt_safari_zone` = Madla/Hinouta/Akola). The installer also writes the **navigation tree** (`navigation` section) to the PWT Navigation options page — items resolve by page/post slug (`get_page_by_path`), so they render only for pages that exist after step 3.
